...
lcdst_t *lcdst_init(int spiSpeed, int cs, int a0, int rs, int w, int h)
{
...
void lcdst_setOrientation(uint8 orientation, uint width, uint height)
{
	writeCommand(0x36); /* Memory Data Access Control */

	activeDisplay->width  = width;
	activeDisplay->height = height;
	switch(orientation)
	{
		case 1:
			writeData(0x60 | COLOR_MODE); /* MX + MV */
			break;

		case 2:
			writeData(0xC0 | COLOR_MODE); /* MY + MX */
			break;

		case 3:
			writeData(0xA0 | COLOR_MODE); /* MY + MV */
			break;

		default:
			writeData(0x00 | COLOR_MODE); /* None */
			break;
	}
	lcdst_setWindow(0, 0, activeDisplay->width - 1, activeDisplay->height - 1);
...
uint8 lcdst_setWindow(uint8 x1, uint8 y1, uint8 x2, uint8 y2)
{
	...
	/* Set column address */
	writeCommand(0x2A);
	writeData(0); writeData(x1 + SCREENOFFSET_X);
	writeData(0); writeData(x2 + SCREENOFFSET_X);
	
	/* Set row address */
	writeCommand(0x2B);
	writeData(0); writeData(y1 + SCREENOFFSET_Y);
	writeData(0); writeData(y2 + SCREENOFFSET_Y);
